...
test("truth: migration seed JSON matches the canonical template maps exactly", () => {
  ...
  for (const roleKey of ROLE_KEYS) {
    ...
    // serialized map must appear verbatim in migration text. The latest seed
    // refresh is 20260915024000_integrated_permissions.sql, regenerated by
    // scripts/generate-permission-migration.ts whenever PERMISSION_KEYS or a
    // template pack changes (HR hub keys, 2026-09-16). The DPM-merge
    // migration's program_manager row is frozen history and is intentionally
    // NOT the exact-match target anymore.
    assert.ok(
      seedSql.includes(canonical),

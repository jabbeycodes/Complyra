...
-- --------------------------------------------------------------------------
-- Template-library and draft-save RPCs appended 2026-09-14 (RLS-write
-- invariant). The hosted client previously wrote delegation_templates and
-- delegation_training_materials directly, but this migration ships no
-- INSERT/UPDATE/DELETE table policies, so those writes were rejected by RLS.
-- These SECURITY DEFINER RPCs close the gap: every write in this feature now
-- goes through an RPC, each gated on its permission.
-- --------------------------------------------------------------------------

create or replace function public.create_delegation_template(
  p_agency_id uuid,
  p_name text,
  p_category text,
  p_sections jsonb,
  p_individualization_note text
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if not private.has_permission(p_agency_id, 'delegation.templates.manage') then
    raise exception 'You do not have permission to do that.';
  end if;
  ...
  insert into public.delegation_templates
    (agency_id, name, category, sections, individualization_note, active)
  values
    (p_agency_id, btrim(p_name), p_category,
     coalesce(p_sections, '{}'::jsonb), coalesce(p_individualization_note, ''), true)
...
create or replace function public.update_delegation_template(
  p_template_id uuid,
...
declare
  v_tpl public.delegation_templates%rowtype;
begin
  select * into v_tpl from public.delegation_templates where id = p_template_id;
  if v_tpl.id is null then
    raise exception 'Delegation template not found.';
  end if;
  if v_tpl.agency_id is null then
    -- Common-library template: editable by a caller holding
    -- delegation.templates.manage in any of their agencies (local-API parity:
    -- the local client also allows editing the shared library).
    if not exists (
      select 1
      from public.memberships m
      join public.agency_roles ar
      ...
      where m.user_id = auth.uid()
        and (m.expires_on is null or m.expires_on >= current_date)
        and coalesce((ar.permissions ->> 'delegation.templates.manage')::boolean, false)
    ) then
      raise exception 'You do not have permission to do that.';
    end if;
  elsif not private.has_permission(v_tpl.agency_id, 'delegation.templates.manage') then
    raise exception 'You do not have permission to do that.';
  end if;

  update public.delegation_templates
...
create or replace function public.save_delegation_training_draft(
  p_assignment_id uuid,
...
begin
  ...
  if v_mat.status not in ('draft', 'in_review') then
    raise exception 'Only a draft or in-review material can be edited.';
  end if;

  update public.delegation_training_materials
  set draft_content = coalesce(p_draft_content, '{}'::jsonb)
      || '{"generatedMark": "Digital record generated by Complyrer."}'::jsonb
  where id = v_mat.id
  returning * into v_mat;
